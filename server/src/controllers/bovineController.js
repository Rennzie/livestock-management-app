...
function bovineCategoryUpdate(req, res, next) {
  Bovine.update({ _id: { $in: req.body.ids } }, { category: req.body.newCategory }, { multi: true })
...
function bovineWeightAdd(req, res, next) {
  Bovine.findById(req.params.bovineId)
...
function bovineWeightsBatchUpload(req, res, next) {
  ...
  const fileRows = [];
  csv
    .fromPath(req.file.path, csvOptions)
    .on('data', data => {
